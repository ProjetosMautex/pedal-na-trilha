        ...
        <div className="my-16">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
            {accessories.map((item) => (
              <div key={item.id} className="bg-zinc-900 border border-zinc-700 rounded-xl p-4 flex flex-col items-center text-center hover:border-cyan-500 transition-colors">
                <img src={item.imageUrl} alt={item.name} className="w-32 h-32 object-contain mb-4 bg-white rounded-lg p-2" />
                <h4 className="text-white font-bold mb-2 h-10 line-clamp-2">{item.name}</h4>
                <a 
                  href={item.affiliateUrl} 
                  target="_blank" 
                  rel="noopener noreferrer"
                  className="w-full bg-cyan-600 hover:bg-cyan-500 text-white font-bold py-2 px-4 rounded transition-colors mt-auto"
                >
                  Ver Preço
                </a>
              </div>
