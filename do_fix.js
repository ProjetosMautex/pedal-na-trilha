...
// 1. Remove Torquimetro (Wait, Torquimetro is already NOT in MelhorGuidao.tsx, remember?! I checked earlier and it was missing because the injection regex failed!)
// Let me verify if Torquimetro is in MelhorGuidao.tsx right now.
const torqRegex = /<div className="my-12 p-6 bg-zinc-900\/80 rounded-2xl border border-zinc-700 shadow-xl">[\s\S]*?<AffiliateCard product=\{accessories\.find\(p => p\.id === 'torquimetro-estalo'\)!\}\s*\/>\s*<\/div>/;
text = text.replace(torqRegex, '');

// 2. Remove Manopla
const manoplaRegex = /<div className="my-10 p-6 bg-zinc-900\/50 rounded-2xl border border-zinc-700 shadow-xl">\s*<h4 className="text-lg font-bold text-white mb-4">Complete o combo para zerar as dores nas mãos:<\/h4>\s*<AffiliateCard product=\{accessories\.find\(p => p\.id === 'manopla-gta-lock'\)!\}\s*\/>\s*<\/div>/;
text = text.replace(manoplaRegex, '');

// 3. Remove Mesa
const mesaRegex = /\{faq\.question\.includes\("mesa"\) && \(\s*<div className="mt-6 bg-zinc-900\/50 p-4 rounded-xl border border-zinc-700">\s*<h4 className="text-lg font-bold text-cyan-400 mb-4">Opção de mesa curta \(35mm\) para guidões agressivos:<\/h4>\s*<AffiliateCard product=\{accessories\.find\(p => p\.id === 'mesa-absolute-35mm'\)!\}\s*\/>\s*<\/div>\s*\)\}/;
text = text.replace(mesaRegex, '');

// 4. Replace Banner
const bannerRegex = /\{\/\* CROSS-SELL BANNER \*\/\}[\s\S]*?\{\/\* FAQ \*\/}/;
...
console.log('MelhorGuidao.tsx fixed!');
